      ...
      <div className="carousel">
        {featured.map((art, i) => (
          <div key={art.slug || i} className={'carousel-slide' + (i === active ? ' active' : '')}>
            <a href={'/artigo/' + art.slug} style={{ display: 'block', position: 'relative', textDecoration: 'none' }}>
              <img src={art.image || '/placeholder.jpg'} alt={art.title} className="carousel-img" />
              <div className="carousel-overlay" />
              <div className="carousel-content">
                <span className="carousel-cat" style={{ background: CAT_COLORS[art.category] || '#555' }}>
                  {art.category}
                </span>
        ...
        <div className="carousel-controls">
          ...
          <div className="carousel-dots">
            {featured.map((_, i) => (
              <button key={i} className={'carousel-dot' + (i === active ? ' active' : '')} onClick={() => go(i)} aria-label={'Slide ' + (i + 1)} />
            ))}
          </div>
          <button className="carousel-btn" onClick={() => go(active + 1)} aria-label="Proximo">&#8250;</button>
        </div>
      </div>
      <div className="hero-list">
        <div className="hero-list-header">Mais Noticias</div>
        {listItems.map((art, i) => (
          <a key={art.slug || i} href={'/artigo/' + art.slug} className="hero-list-item">
            <img src={art.image || '/placeholder.jpg'} alt={art.title} className="hero-list-img" />
            <div className="hero-list-info">
              <span className="hero-list-cat" style={{ color: CAT_COLORS[art.category] || '#00b4a2' }}>{art.category}</span>
