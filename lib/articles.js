// lib/articles.js
// FunÃ§Ãµes para ler e salvar artigos
// Os artigos ficam no arquivo data/articles.json do repositÃ³rio GitHub
...
// LÃª todos os artigos (do arquivo local)
...
export function getLatestArticles(limit = 6) {
  ...
  return all.filter(a => (a.slug || a.id) !== (featured?.slug || featured?.id)).slice(0, limit)
}

// Gera um slug a partir de um tÃ­tulo
